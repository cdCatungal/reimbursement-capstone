...
export async function sendEmail(to, subject, html, cc = null) {
  try {
    ...
    if (error.code === "EAUTH") {
      ...
      console.error(
        "❌ Connection failed. Check your internet connection and email service settings"
      );
      console.error(
        "💡 Make sure EMAIL_SERVICE is set correctly (gmail/outlook)"
      );
    } else if (error.code === "ETIMEDOUT") {
      console.error(
        "❌ Connection timeout. Your email service may be blocked or slow"
      );
    }
