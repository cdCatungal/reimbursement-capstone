import { User, SapCode, UserSapCode, Reimbursement, Approval } from "../models/index.js";
...
export const userSettings = async (req, res) => {
  try {
    ...
    const user = await User.findByPk(req.user.id, {
      attributes: ["id", "email", "name", "role", "profilePicture", "authProvider", "isActive"],
      include: [
        {
          model: SapCode,
          as: "sapCodes",
...
export const getAllUsers = async (req, res) => {
  try {
    ...
    const users = await User.findAll({
      attributes: ["id", "email", "name", "role", "profilePicture", "authProvider", "isActive", "assigned_sul_id"],
      include: [
        {
          model: SapCode,
          as: "sapCodes",
...
export const updateUser = async (req, res) => {
  try {
    ...
    if (assigned_sul_id !== undefined && 
      ...
      console.log(`🔄 SUL assignment changing for ${user.name}: ${oldAssignedSulId} → ${assigned_sul_id}`);
      ...
      if (pendingSulApprovals.length > 0 && assigned_sul_id !== null) {
        console.log(`📋 Found ${pendingSulApprovals.length} pending SUL approvals to reassign`);
