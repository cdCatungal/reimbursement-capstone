...
function AdminDashboard() {
  ...
  const handleApprove = async (id) => {
    try {
      const response = await fetch(
        `${process.env.REACT_APP_API_URL}/api/reimbursements/${id}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
                    ...
                    <ListItemText
                      ...
                      secondary={
                        <>
                          ...
                          >
                            User:{" "}
                            {item.user?.displayName || item.userId || "Unknown"}
                          </Typography>
                          <br />
                          <Typography
                            component="span"
      ...
      >
        ...
        >
          ...
          <Box sx={{ display: "flex", alignItems: "center", gap: 2 }}>
            ...
            >
              <MenuItem
                selected={tabValue === "user-profile"}
                onClick={() => {
        ...
        <Box sx={{ p: 2 }}>
          ...
          >
            <DialogTitle sx={{ fontWeight: "bold" }}>
              {selectedTicket
                ? `${selectedTicket.category} Reimbursement Details`
                : "Details"}
            </DialogTitle>
            <DialogContent>
              {selectedTicket ? (
                <Box sx={{ display: "flex", flexDirection: "column", gap: 2 }}>
                  ...
                  <Typography>
                    <strong>Amount:</strong> ₱{selectedTicket.total}
                  </Typography>
                  <Typography
                    sx={{ color: getStatusColor(selectedTicket.status) }}
                  >
                  ...
                  <Typography>
                    ...
                    {selectedTicket.merchant || "N/A"}
                  </Typography>
                  <Typography>
                    <strong>Items:</strong> {selectedTicket.items || "N/A"}
                  </Typography>
                  <Typography>
                    <strong>Date:</strong>{" "}
                  ...
                  {selectedTicket.extractedText && (
                    <Typography>
                      ...
                      <pre>{selectedTicket.extractedText}</pre>
                    </Typography>
                  )}
                  {selectedTicket.receipt ? (
                    ...
                    <Typography>
                      <strong>Receipt:</strong> No receipt uploaded
                    </Typography>
                  )}
                </Box>
              ) : (
