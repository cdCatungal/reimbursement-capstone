...
function ReportExport({ reimbursements = [] }) {
  ...
  const filterData = () => {
    return reimbursements.filter((item) => {
      const itemDate = new Date(item.date || item.submittedAt);
  ...
  const exportToExcel = async () => {
    ...
    const filename = `reimbursement_report_${new Date().toISOString().split('T')[0]}.xlsx`;
            ...
            <Table stickyHeader size="small">
              <TableHead>
                <TableRow>
                  ...
                  <TableCell sx={{ fontWeight: 'bold', bgcolor: 'primary.main', color: 'primary.contrastText' }} align="right">
                    Amount
                  </TableCell>
                  <TableCell sx={{ fontWeight: 'bold', bgcolor: 'primary.main', color: 'primary.contrastText' }}>
                    Status
