...
Please review all details carefully before submitting. Once submitted, your request will be sent to the approval workflow.
</Alert>
...
<Typography variant="caption" color="text.secondary">
Image • {image ? (image.size / 1024 / 1024).toFixed(2) : '0'} MB
</Typography>
</Box>
</>
) : null}
...
<Typography variant="body1" sx={{ fontWeight: 600 }}>
{formData.number_of_people} {parseInt(formData.number_of_people) === 1 ? 'person' : 'people'}
</Typography>
</Grid>
)}
...
<Typography variant="body1" sx={{ fontWeight: 600 }}>
{formData.number_of_days} {parseInt(formData.number_of_days) === 1 ? 'day' : 'days'}
</Typography>
</Grid>
<Grid item xs={12} sm={6}>
<Typography variant="caption" color="text.secondary">
...
Your total amount exceeds the category limit. Only ₱{reimbursableAmount.toFixed(2)} will be reimbursed.
</Alert>
)}
</Paper>
</Grid>
...
{['Overtime Meal', 'Meal with Client', 'Accommodation'].includes(formData.category) && formData.total && (
<Box
sx={{
p: 2,
bgcolor: 'grey.100',
...
disabled={loading || (!bypassesSapValidation && availableSapCodes.length === 0) || submitting}
>
